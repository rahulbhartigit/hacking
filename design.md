...
CREATE INDEX idx_channels_sync_status ON channels(sync_status);
```
...
CREATE INDEX idx_video_metrics_date ON video_metrics(date DESC);
```
...
CREATE INDEX idx_retention_video_id ON retention_data(video_id);
```
...
CREATE TABLE insights (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  channel_id UUID REFERENCES channels(id) ON DELETE CASCADE,
  type VARCHAR(50) NOT NULL,  -- 'video_idea', 'title', 'thumbnail', 'hook', 'retention', 'diagnostic'
  title VARCHAR(500) NOT NULL,
  content JSONB NOT NULL,  -- Flexible structure for different insight types
...
CREATE INDEX idx_insights_channel_id ON insights(channel_id);
CREATE INDEX idx_insights_type ON insights(type);
CREATE INDEX idx_insights_created_at ON insights(created_at DESC);
CREATE INDEX idx_insights_content ON insights USING GIN(content);
```
...
CREATE TABLE subscriptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  tier VARCHAR(50) NOT NULL,  -- 'free', 'pro', 'agency'
...
CREATE TABLE usage_tracking (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  feature VARCHAR(100) NOT NULL,  -- 'insights_generated', 'api_calls'
...
CREATE INDEX idx_usage_user_feature ON usage_tracking(user_id, feature);
...
CREATE TABLE jobs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  type VARCHAR(100) NOT NULL,  -- 'analytics_sync', 'ai_generation'
...
```typescript
async function validateApiKey(keyString: string): Promise<User | null> {
...
const result = await generateVideoIdeas(channelId);
const duration = Date.now() - startTime;

await cloudwatch.putMetricData({
  Namespace: 'YouTubeGrowth',
  MetricData: [{
    MetricName: 'AIGenerationDuration',
    Value: duration,
    Unit: 'Milliseconds',
    Dimensions: [{
      Name: 'InsightType',
